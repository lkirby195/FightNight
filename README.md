...
pipeline/betting_system.py  KISS betting rule (flip>=65% + gap>=100pts, flat 1u).
                        Usage: betting_system.py [year|all] [--write-ledger]
```
...
data/placeable_lines.csv    current BFO line per bout, captured by
                            card_report.py when a future event is reported (one
                            row per bout per capture day); the bet side's
                            earliest capture is the ledger's placeable_line
data/system_ledger.csv      ledger of record. GENERATED by
                            ...
                            units, open_line, placeable_line, clean_close,
                            clv_pts, result, pnl, pnl_at_open
